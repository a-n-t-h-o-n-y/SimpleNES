        ...
        if (m_writeCounter == 5) {
            if (addr <= 0x9fff) {
                ...
                else  // two 4KB banks
                {
                    ...
                    m_secondBankCHR =
                        &m_cartridge.getVROM()[0x1000 * m_regCHR1];
                }
            }
            else if (addr <= 0xbfff)  // CHR Reg 0
            {
                ...
                if (m_modeCHR == 1)
                    m_secondBankCHR =
                        &m_cartridge.getVROM()[0x1000 * m_tempRegister];
            }
            else {
                // TODO PRG-RAM
...
void MapperSxROM::calculatePRGPointers()
{
    ...
    else  // switch first fix second
    {
        ...
        m_secondBankPRG = &m_cartridge.getROM()[m_cartridge.getROM().size() -
                                                0x4000 /*0x2000 * 0x0e*/];
    }
}
...
void MapperSxROM::writeCHR(Address addr, Byte value)
{
    ...
    // else
    //     LOG(Info) << "Read-only CHR memory write attempt at " << std::hex
    //               << addr << std::endl;
}
}  // namespace sn
