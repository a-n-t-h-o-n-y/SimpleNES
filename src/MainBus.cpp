...
Byte MainBus::read(Address addr)
{
    if (addr < 0x2000)
        ...
        {
            auto it =
                m_readCallbacks.find(static_cast<IORegisters>(addr & 0x2007));
        ...
        // else
        //     LOG(InfoVerbose)
        //         << "Read access attempt at: " << std::hex << +addr <<
        //         std::endl;
    }
    // else if (addr < 0x6000) {
    // LOG(InfoVerbose)
...
void MainBus::write(Address addr, Byte value)
{
    if (addr < 0x2000)
        ...
        {
            auto it =
                m_writeCallbacks.find(static_cast<IORegisters>(addr & 0x2007));
        ...
        // else
        //     LOG(InfoVerbose) << "Write access attmept at: " << std::hex <<
        //     +addr
        //                      << std::endl;
    }
    // else if (addr < 0x6000) {
