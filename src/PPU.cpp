...
PPU::PPU(PictureBus& bus) : m_bus{bus}, m_spriteMemory(64 * 4, 0) {}
    ...
    switch (m_pipelineState) {
        ...
        case Render:
            if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
                ...
                auto bgOpaque         = false;
                auto sprOpaque        = true;
                auto spriteForeground = false;

                auto const x = m_cycle - 1;
                auto const y = m_scanline;

                if (m_showBackground) {
                    auto const x_fine = (m_fineXScroll + x) % 8;
                    if (!m_hideEdgeBackground || x >= 8) {
                        // fetch tile
                        // mask off fine y
                        auto addr       = 0x2000 | (m_dataAddress & 0x0FFF);
                        auto const tile = read(addr);

                        // fetch pattern
                        // Each pattern occupies 16 bytes, so multiply by 16
                        addr = (tile * 16) + ((m_dataAddress >> 12 /*y % 8*/) &
                                              0x7);  // Add fine y
                        addr |= m_bgPage << 12;  // set whether the pattern is
                                                 // in the high or low page
                        ...
                        bgOpaque = bgColor;  // flag used to calculate final
                                             // pixel with the sprite pixel

                        // fetch attribute and calculate higher two bits of
                        // palette
                        addr = 0x23C0 | (m_dataAddress & 0x0C00) |
                               ((m_dataAddress >> 4) & 0x38) |
                               ((m_dataAddress >> 2) & 0x07);
                        auto const attribute = read(addr);
                        auto const shift =
                            ((m_dataAddress >> 4) & 4) | (m_dataAddress & 2);
                ...
                if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
                    for (auto i : m_scanlineSprites) {
                        auto const spr_x = m_spriteMemory[i * 4 + 3];

                        if (0 > (x - spr_x) || (x - spr_x) >= 8)
                            continue;

                        auto const spr_y     = m_spriteMemory[i * 4 + 0] + 1;
                        auto const tile      = m_spriteMemory[i * 4 + 1];
                        auto const attribute = m_spriteMemory[i * 4 + 2];

                        auto const length = m_longSprites ? 16 : 8;

                        auto x_shift  = (x - spr_x) % 8;
                        auto y_offset = (y - spr_y) % length;
                        ...
                        {
                            // bit-3 is one if it is the bottom tile of the
                            // sprite, multiply by two to get the next pattern
                        ...
                        if (!m_sprZeroHit && m_showBackground && i == 0 &&
                            sprOpaque && bgOpaque) {
                            m_sprZeroHit = true;
                        }
                        // Exit the loop now since we've found the highest
                        // priority sprite
                        break;
                    }
                }

                auto paletteAddr = bgColor;
                ...
                if ((!bgOpaque && sprOpaque) ||
                    ...
                    paletteAddr = 0;
                framebuffer_[x][y] = m_bus.readPalette(paletteAddr);
            }
                ...
                if ((m_dataAddress & 0x7000) != 0x7000)  // if fine Y < 7
                    ...
                    auto y = (m_dataAddress & 0x03E0) >> 5;  // let y = coarse Y
                    if (y == 29) {
                        y = 0;                    // coarse Y = 0
            ...
            if (m_cycle >= ScanlineEndCycle) {
                ...
                // This isn't where/when this indexing, actually copying in 2C02
                // is done but (I think) it shouldn't hurt any games if this is
                // done here
                ...
                for (std::size_t i = m_spriteDataAddress / 4; i < 64; ++i) {
                    auto const diff = (m_scanline - m_spriteMemory[i * 4]);
...
void PPU::doDMA(const Byte* page_ptr)
{
    ...
    if (m_spriteDataAddress) {
        std::memcpy(m_spriteMemory.data(),
