...
void CPU::step()
{
    ...
    // int psw =
    //     f_N << 7 | f_V << 6 | 1 << 5 | f_D << 3 | f_I << 2 | f_Z << 1 | f_C;
    ...
    // failed ExecuteImplied must be called first and ExecuteBranch must be
    // before ExecuteType0
...
bool CPU::executeImplied(Byte opcode)
{
    switch (static_cast<OperationImplied>(opcode)) {
        ...
        case RTI: {
            ...
            r_PC |= pullStack() << 8;
            break;
        case JMP: r_PC = readAddress(r_PC); break;
        case JMPI: {
            Address location = readAddress(r_PC);
            // 6502 has a bug such that the when the vector of anindirect
            // address begins at the last byte of a page, the second byte is
            // fetched from the beginning of that page rather than the beginning
            // of the next Recreating here:
        ...
        case PHP: {
            Byte flags =
                f_N << 7 | f_V << 6 | 1 << 5 |  // supposed to always be 1
        ...
        case INX:
            ...
            setZN(r_X);
            break;
        case CLC: f_C = false; break;
        case SEC: f_C = true; break;
        case CLI: f_I = false; break;
        case SEI: f_I = true; break;
        case CLD: f_D = false; break;
        case SED: f_D = true; break;
        case TYA:
            r_A = r_Y;
            setZN(r_A);
            break;
        case CLV: f_V = false; break;
        case TXA:
            r_A = r_X;
            setZN(r_A);
            break;
        case TXS: r_SP = r_X; break;
        case TAX:
            r_X = r_A;
            setZN(r_X);
            break;
...
bool CPU::executeBranch(Byte opcode)
{
    if ((opcode & BranchInstructionMask) == BranchInstructionMaskResult) {
        ...
        switch (opcode >> BranchOnFlagShift) {
            case Negative: branch = !(branch ^ f_N); break;
            case Overflow: branch = !(branch ^ f_V); break;
            case Carry: branch = !(branch ^ f_C); break;
            case Zero: branch = !(branch ^ f_Z); break;
            default: return false;
        }
...
bool CPU::executeType1(Byte opcode)
{
    if ((opcode & InstructionModeMask) == 0x1) {
        ...
        switch (
            static_cast<AddrMode1>((opcode & AddrModeMask) >> AddrModeShift)) {
            ...
            case ZeroPage: location = m_bus.read(r_PC++); break;
            case Immediate: location = r_PC++; break;
            case Absolute:
        ...
        switch (op) {
            ...
            case STA: m_bus.write(location, r_A); break;
            case LDA:
                r_A = m_bus.read(location);
                setZN(r_A);
                break;
...
bool CPU::executeType2(Byte opcode)
{
    if ((opcode & InstructionModeMask) == 2) {
        ...
        switch (addr_mode) {
            case Immediate_: location = r_PC++; break;
            case ZeroPage_: location = m_bus.read(r_PC++); break;
        ...
        switch (op) {
            ...
            case ROR:
                ...
                break;
            case STX: m_bus.write(location, r_X); break;
            case LDX:
...
bool CPU::executeType0(Byte opcode)
{
    if ((opcode & InstructionModeMask) == 0x0) {
        ...
        std::uint16_t operand = 0;
        switch (static_cast<Operation0>((opcode & OperationMask) >>
                                        OperationShift)) {
            case BIT:
                operand = m_bus.read(location);
                f_Z     = !(r_A & operand);
                ...
                f_N     = operand & 0x80;
                break;
            case STY: m_bus.write(location, r_Y); break;
            case LDY:
