      ...
      m_cpuCycleDuration(std::chrono::nanoseconds(559))
{
    ...
    //     !m_bus.setReadCallback(PPUDATA,
    //                            [&](void) { return m_ppu.getData(); }) ||
    //     // TODO these will be calls to your own controller implementation.
    ...
    //     !m_bus.setWriteCallback(PPUSCROL,
    //                             [&](Byte b) { m_ppu.setScroll(b); }) ||
    ...
    //                             }) ||
    //     !m_bus.setWriteCallback(OAMDATA,
    //                             [&](Byte b) { m_ppu.setOAMData(b); })) {
    //     LOG(Error) << "Critical error: Failed to set I/O callbacks"
    //                << std::endl;
    // }
...
// m_elapsedTime = m_cycleTimer - m_cycleTimer;  // always zero
...
// if (focus && !pause) {
//     m_elapsedTime +=
//         std::chrono::high_resolution_clock::now() - m_cycleTimer;
//     m_cycleTimer = std::chrono::high_resolution_clock::now();
...
//               << " set. Screen: " << int(NESVideoWidth * m_screenScale) <<
//               "x"
